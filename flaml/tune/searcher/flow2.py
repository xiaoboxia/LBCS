...
class FLOW2(Searcher):
    ...
    def lexico_compare(self, result) -> bool:
        if self._histories is None:
            ...
            # if self._f_best[self.lexico_objectives["metrics"][0]] < self.lexico_objectives["tolerances"]["lower_bound"]:
            #     k_metric = self.lexico_objectives["metrics"][1]
            #     return result[k_metric] < self.best_obj[k_metric] and result[self.lexico_objectives["metrics"][0]] < self.lexico_objectives["tolerances"]["lower_bound"]
    ...
    def rand_vector_unit_sphere(self, dim, trunc=0) -> np.ndarray:
        vec = self._random.normal(-0.1, 1, dim)
        if 0 < trunc < dim:
